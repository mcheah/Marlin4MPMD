...
#if MB(STM_3DPRINT)
#define MultiU24X24toH16(intRes, longIn1, longIn2) intRes = (uint16_t)((((uint64_t)longIn1 * (uint64_t)longIn2)>> 24)& 0XFFFF);
#else // #if MB(STM_3DPRINT)
...
//      #endif
  ...
  if (current_block != NULL) {
    ...
    if ((((out_bits & (1<<X_AXIS)) != 0)&&(out_bits & (1<<Y_AXIS)) != 0)) {   //-X occurs for -A and -B
      ...
      CHECK_ENDSTOPS
      {
        ...
        #endif          
        {
            ...
            bool x_min_endstop=((bool)READ(X_MIN_PIN) != X_MIN_ENDSTOP_INVERTING);
    ...
    if ((((out_bits & (1<<X_AXIS)) != 0)&&(out_bits & (1<<Y_AXIS)) == 0)) {   // -Y occurs for -A and +B
      ...
      {
        #if defined(Y_MIN_PIN) && Y_MIN_PIN > -1
          bool y_min_endstop=((bool)READ(Y_MIN_PIN) != Y_MIN_ENDSTOP_INVERTING);
      ...
      {
        #if defined(Y_MAX_PIN) && Y_MAX_PIN > -1
          bool y_max_endstop=((bool)READ(Y_MAX_PIN) != Y_MAX_ENDSTOP_INVERTING);
    ...
    if ((out_bits & (1<<Z_AXIS)) != 0) {   // -direction
      ...
      CHECK_ENDSTOPS
      {
          ...
          bool z_min_endstop=((bool)READ(Z_MIN_PIN) != Z_MIN_ENDSTOP_INVERTING);
	 ...
	 * lag to allow it work with without needing NOPs */ 
      ...
      if (counter_z > 0) {
        ...
        #endif        
          counter_x -= current_block->step_event_count;
          signed char tmp_count_direction = count_direction[X_AXIS]; 
          count_position[X_AXIS]+=tmp_count_direction;   
        #ifdef DUAL_X_CARRIAGE
          if (extruder_duplication_enabled){
            WRITE(X_STEP_PIN, INVERT_X_STEP_PIN);
            WRITE(X2_STEP_PIN, INVERT_X_STEP_PIN);
        ...
        if (counter_y > 0) {
          ...
          counter_y -= current_block->step_event_count;
          signed char tmp_count_direction = count_direction[Y_AXIS];
          count_position[Y_AXIS]+=tmp_count_direction;
          WRITE(Y_STEP_PIN, INVERT_Y_STEP_PIN);
      ...
      if (counter_z > 0) {
        ...
        counter_z -= current_block->step_event_count;
        signed char tmp_count_direction = count_direction[Z_AXIS];
        count_position[Z_AXIS]+=tmp_count_direction;
        WRITE(Z_STEP_PIN, INVERT_Z_STEP_PIN);
        ...
        if (counter_e > 0) {
          ...
          counter_e -= current_block->step_event_count;
          signed char tmp_count_direction = count_direction[E_AXIS];
          count_position[E_AXIS]+=tmp_count_direction;
          WRITE_E_STEP(INVERT_E_STEP_PIN);
        }
      #endif //!ADVANCE
      #endif
...
#if MB(STM_3DPRINT)
  BSP_MiscTickSetPeriod(OCR1A_nominal);
#else //#if MB(STM_3DPRINT)
      OCR1A = OCR1A_nominal;
#endif   //#else if MB(STM_3DPRINT)
