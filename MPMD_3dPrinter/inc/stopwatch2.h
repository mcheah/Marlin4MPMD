...
#ifndef __STOPWATCH2_H_
#define __STOPWATCH2_H_
...
__STATIC_INLINE void StopWatch_DelayMs_From(uint32_t mS, uint32_t startTime )
{
	uint32_t ticks = StopWatch_MsToTicks(mS);
	uint32_t i=0;
	while (StopWatch_Elapsed(startTime) <ticks) { i++; }
	printf("Delay i=%lu\r\n",i);
}
 ...
 * @return	Nothing
 */
__STATIC_INLINE void StopWatch_DelayNs(uint32_t NS)
{
	uint32_t ticks = StopWatch_UsToTicks(NS)/1000;
	uint32_t startTime = StopWatch_Start();
	while (StopWatch_Elapsed(startTime) < ticks) {}
}
 ...
 * @return     Number of ticks elapsed since stopwatch was started
 */
__STATIC_INLINE uint32_t StopWatch_Elapsed_Ms(uint32_t startTime)
{
	return StopWatch_TicksToMs(StopWatch_Start() - startTime);
}
