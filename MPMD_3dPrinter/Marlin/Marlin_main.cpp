...
static void clean_up_after_endstop_or_probe_move() {
  ...
  inline void do_probe_raise(float z_raise) {
    ...
    if (z_dest > delta_clip_start_height)
 ...
 // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
  ...
  //
  static float probe_pt(float x, float y, bool stow = true, int verbose_level = 1,float zprob_off=-9999) {
    #if ENABLED(DEBUG_LEVELING_FEATURE)
      if (DEBUGGING(LEVELING)) {
...
#define DELTA_CALIBRATION_DEFAULT_POINTS 3
  ...
  static void print_G33_settings(const bool end_stops, const bool tower_angles) {
    ...
    inline float calibration_probe(const float nx, const float ny, const bool stow,float zprob_off=-9999) {
      #if HAS_BED_PROBE
//        return probe_pt((float)nx, (float)ny, stow, 0, false);
        return probe_pt((float)nx, (float)ny, stow, 3);
      #else
        UNUSED(stow);
        return lcd_probe_pt(nx, ny);
      #endif
    }
    volatile float XCOORD[7];
    volatile float YCOORD[7];
    static float probe_G33_points(float z_at_pt[NPP + 1], const int8_t probe_points, const bool towers_set, const bool stow_after_each) {
       ...
       if (!_0p_calibration) {
         ...
         if (!_7p_no_intermediates && !_7p_4_intermediates && !_7p_11_intermediates) { // probe the center
           z_at_pt[CEN] += calibration_probe(0, 0, stow_after_each,z_adj_at_pt[CEN]);
         ...
         if (_7p_calibration) { // probe extra center points
           ...
           I_LOOP_CAL_PT(axis, start, steps) {
             const float a = RADIANS(210 + (360 / NPP) *  (axis - 1)),
                         r = DELTA_PROBEABLE_RADIUS * 0.1;
             XCOORD[CEN] = cos(a) * r;
             YCOORD[CEN] = sin(a) * r;
             z_at_pt[CEN] += calibration_probe(cos(a) * r, sin(a) * r, stow_after_each,z_adj_at_pt[CEN]);
             if (isnan(z_at_pt[CEN])) return NAN;
          }
...
//               const float a = RADIANS(210 + (360 / NPP) *  (axis - 1)),
            	 const float a = RADIANS(210 + (360 / NPP) *  (axis - 1)),
                           ...
                           r = DELTA_PROBEABLE_RADIUS * (1 + 0.1 * (zig_zag ? circle : - circle)),
                           interpol = fmod(axis, 1);
               XCOORD[uint8_t(round(axis - interpol))           % NPP + 1] = cos(a) * r;
               YCOORD[uint8_t(round(axis - interpol))           % NPP + 1] = sin(a) * r;
//               const float z_temp = calibration_probe(cos(a) * r, sin(a) * r, stow_after_each);
               float z_temp = probe_pt(cos(a) * r,sin(a) * r,
            		   true,
					   3,
					   z_adj_at_pt[uint8_t(round(axis - interpol)) % NPP + 1]);
    ...
    static bool G33_auto_tune() {
      ...
      SERIAL_EOL;
      if (isnan(probe_G33_points(z_at_pt_base, 3, true, true))) return false;
...
//        if (!home_delta()) return false;
        ...
        if (isnan(probe_G33_points(z_at_pt, 3, true, true))) return false;
...
//      if (!home_delta()) return false;
  ...
  /**
    ...
    inline void gcode_G33() {
      ...
      bool hasP, hasV, hasC, hasF;
      int8_t probe_points = DELTA_CALIBRATION_DEFAULT_POINTS;
      int8_t verbose_level = 1;
      float calibration_precision = 0.0;
      int8_t force_iterations = 0.0;
      bool towers_set           = false,
                 auto_tune            = false,
                 stow_after_each      = false;
      if ((hasP = code_seen('P'))) probe_points = code_value_int();
//    				  parser.intval('P', DELTA_CALIBRATION_DEFAULT_POINTS);
      if (!WITHIN(probe_points, 0, 10)) {
//        SERIAL_PROTOCOLLNPGM("?(P)oints is implausible (0-10).");
        return;
      }
      if ((hasV = code_seen('V'))) verbose_level = code_value_int();
//       = parser.byteval('V', 1);
      if (!WITHIN(verbose_level, 0, 3)) {
//        SERIAL_PROTOCOLLNPGM("?(V)erbose level is implausible (0-3).");
        return;
      }
      if ((hasC = code_seen('C'))) calibration_precision = code_value_float();
//       = parser.floatval('C', 0.0);
      if (calibration_precision < 0) {
//        SERIAL_PROTOCOLLNPGM("?(C)alibration precision is implausible (>=0).");
        return;
      }
      if ((hasF = code_seen('F'))) force_iterations = code_value_int();
//       = parser.intval('F', 0);
      if (!WITHIN(force_iterations, 0, 30)) {
//        SERIAL_PROTOCOLLNPGM("?(F)orce iteration is implausible (0-30).");
        return;
      }
      if ((hasF = code_seen('T'))) towers_set = !code_value_int();
      if ((hasF = code_seen('A'))) auto_tune = code_value_int();
      if ((hasF = code_seen('E'))) stow_after_each = code_value_int();
...
//                 stow_after_each      = parser.boolval('E'),
      ...
      const static char save_message[] PROGMEM = "Save w/ M500";
...
//      SERIAL_PROTOCOLLNPGM("G33 Auto Calibrate");
      ...
      if (!_1p_calibration && !_0p_calibration) {  // test if the outer radius is reachable
        LOOP_CAL_RAD(axis) {
          const float a = RADIANS(210 + (360 / NPP) *  (axis - 1)),
                      r = DELTA_PROBEABLE_RADIUS * (1 + (_7p_9_centre ? 0.1 : 0.0));
...
//  	  probe_pt(0,45,true,4);
        ...
        if (isnan(zero_std_dev)) {
        	SERIAL_PROTOCOLPGM("Good enough");
...
//  	  probe_pt(0,45,true,4);
        ...
        if ((zero_std_dev < test_precision || iterations <= force_iterations) && zero_std_dev > calibration_precision) {
          ...
          const float r_diff = delta_radius - DELTA_PROBEABLE_RADIUS,
                      h_factor = 1 / 6.0 *
                        #ifdef H_FACTOR
                      ...
                      a_factor = 1 / 6.0 *
                        ...
                        #else
                          (66.66 / DELTA_PROBEABLE_RADIUS);               // 0.83 for cal_rd = 80mm
                        #endif
...
//                SERIAL_PROTOCOLPGM("std dev:");
            	                 SERIAL_PROTOCOLPGM("std:");
                SERIAL_PROTOCOL_F(zero_std_dev_min, 3);
              }
            SERIAL_EOL;
            char mess[21];
//            strcpy_P(mess, PSTR("Cal std:"));
            strcpy_P(mess, PSTR("std:"));
  ...
  inline void gcode_M851() {
    ...
    if (code_seen('Z')) {
      ...
      if (Z_PROBE_OFFSET_RANGE_MIN <= value && value <= Z_PROBE_OFFSET_RANGE_MAX) {
    	...
    	int8_t index = code_value_int();
    	float adj = code_value_axis_units(Z_AXIS);
    	z_adj_at_pt[index] = adj;
    }
    else {
      SERIAL_ECHOPAIR(": ", zprobe_zoffset);
	  SERIAL_ECHO("ZCAL off:");
      for(int k=0;k<NPP+1;k+1)
      {
