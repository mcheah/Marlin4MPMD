...
float probing_feedrate = Z_PROBE_SPEED_FAST;
float probing_z_raise = Z_RAISE_BETWEEN_PROBINGS;
#if ENABLED(PROBE_DOUBLE_TOUCH)
bool double_touch = true;
#else
...
// Extruder offsets
  ...
  float delta_tower1_x = arm_cos_f32(RADIANS(210-120 + delta_tower_angle_trim[A_AXIS])) * (delta_radius + delta_radius_trim_tower_1); // front left tower
  float delta_tower1_y = arm_sin_f32(RADIANS(210-120 + delta_tower_angle_trim[A_AXIS])) * (delta_radius + delta_radius_trim_tower_1);
  float delta_tower2_x = arm_cos_f32(RADIANS(330-120 + delta_tower_angle_trim[B_AXIS])) * (delta_radius + delta_radius_trim_tower_2); // front right tower
  float delta_tower2_y = arm_sin_f32(RADIANS(330-120 + delta_tower_angle_trim[B_AXIS])) * (delta_radius + delta_radius_trim_tower_2);
  float delta_tower3_x = arm_cos_f32(RADIANS( 90-120 + delta_tower_angle_trim[C_AXIS])) * (delta_radius + delta_radius_trim_tower_3); // back middle tower
  float delta_tower3_y = arm_sin_f32(RADIANS( 90-120 + delta_tower_angle_trim[C_AXIS])) * (delta_radius + delta_radius_trim_tower_3);
...
static void update_software_endstops(AxisEnum axis) {
  ...
  #if ENABLED(DELTA)
    if (axis == Z_AXIS) {
      delta_clip_start_height = min(sw_endstop_max[axis],delta_height) - delta_safe_distance_from_top();
    }
  #endif
...
inline void set_destination_to_current() { memcpy(destination, current_position, sizeof(destination)); }
  ...
  void prepare_move_to_destination_raw() {
    ...
    inverse_kinematics(destination);
	  ...
	  if (!bed_leveling_in_progress) adjust_delta(destination);
	#endif
...
static void clean_up_after_endstop_or_probe_move() {
 ...
 // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
  ...
  static float run_z_probe(int verbose_level = 0) {
	long start_pos[3];
	long probe_pos[3];
	float start_raw[3];
	float start_z_logical;
    ...
    refresh_cmd_timeout();
      ...
      if (verbose_level > 3) {
      	SERIAL_PROTOCOLPGM("Start Position: ");
	  //Get stepper counts for reporting only
        LOOP_XYZ(AXIS)
      		start_pos[AXIS] = stepper.position((AxisEnum)AXIS);
      	report_current_position2(current_position,start_pos);
      }
	  //Get position based on stepper counts and save in start_raw
      set_cartesian_from_steppers();
      LOOP_XYZ(AXIS)
      	  start_raw[AXIS] = LOGICAL_POSITION(cartesian_position[AXIS],AXIS);
	  //Get logical position as reported by M114
      start_z_logical = LOGICAL_POSITION(current_position[Z_AXIS],Z_AXIS);
      if (verbose_level > 3) {
	      SERIAL_PROTOCOLPGM("Actual Start Position: ");
	      report_current_position2(start_raw,start_pos);
	  }
      if(double_touch) {
      ...
      if (verbose_level > 3) {
			...
			//Get first probe position, only on double touch for reporting
  	        LOOP_XYZ(AXIS)
  				probe_pos[AXIS] = stepper.position((AxisEnum)AXIS);
  			report_current_position2(current_position,probe_pos);
      }
      // move up the retract distance
      do_blocking_move_to_z(current_position[Z_AXIS] + home_bump_mm(Z_AXIS), XY_PROBE_FEEDRATE_MM_M);
      }
      else {
      // move fast, close to the bed
      do_blocking_move_to_z(home_bump_mm(Z_AXIS), probing_feedrate);
      }
    ...
    if (verbose_level > 3) {
			...
			LOOP_XYZ(AXIS)
				probe_pos[AXIS] = stepper.position((AxisEnum)AXIS);
			report_current_position2(current_position,probe_pos);
			SERIAL_PROTOCOLPGM("Probe difference");
			SERIAL_PROTOCOLPGM(MSG_COUNT_X);
			SERIAL_PROTOCOL(start_pos[X_AXIS]-probe_pos[X_AXIS]);
			SERIAL_PROTOCOLPGM(" y :");
			SERIAL_PROTOCOL(start_pos[Y_AXIS]-probe_pos[Y_AXIS]);
			SERIAL_PROTOCOLPGM(" z :");
			SERIAL_PROTOCOL(start_pos[Z_AXIS]-probe_pos[Z_AXIS]);
			SERIAL_EOL;
    }
    #if ENABLED(DEBUG_LEVELING_FEATURE)
      if (DEBUGGING(LEVELING)) DEBUG_POS("run_z_probe", current_position);
    #endif
	//Error is inverted so that a higher value indicates a higher position, sign matches bed_level matrix
	//Error = expected distance traveled - actual distance traveled
    return (start_z_logical - 0) - (start_raw[Z_AXIS] - current_position[Z_AXIS]);
  }
  ...
  static float probe_pt(float x, float y, bool stow = true, int verbose_level = 1) {
    ...
    do_probe_raise(probing_z_raise);
    ...
    float measured_z = run_z_probe(verbose_level);
    ...
    if (verbose_level > 2) {
      ...
      SERIAL_PROTOCOL_F(x, 5);
      SERIAL_PROTOCOLPGM(" Y:");
      SERIAL_PROTOCOL_F(y, 5);
      SERIAL_PROTOCOLPGM(" Z:");
      SERIAL_PROTOCOL_F(measured_z, 5);
      SERIAL_EOL;
    }
...
#if ENABLED(POLAR_MESH) ||AUTO_BED_LEVELING_GRID_POINTS!=3
...
inline void gcode_G28() {
    ...
    switch (state) {
      ...
      case MeshNext:
        ...
        if (probe_point < (MESH_NUM_X_POINTS) * (MESH_NUM_Y_POINTS)) {
          ...
          current_position[Z_AXIS] = MESH_HOME_SEARCH_Z
            #if probing_z_raise > MIN_Z_HEIGHT_FOR_HOMING
              + probing_z_raise
            #elif MIN_Z_HEIGHT_FOR_HOMING > 0
              + MIN_Z_HEIGHT_FOR_HOMING
            #endif
  ...
  inline void gcode_G29() {
    ...
    if(do_height_probe)
    {
    	if(do_mesh_probe && !dryrun)//clear bed level before probing delta height
    		reset_bed_level();
    ...
    if(!dryrun)
	...
	#else //POLAR_MESH
		  const int thGridSpacing = (right_probe_bed_position - left_probe_bed_position) / (auto_bed_leveling_theta_grid_points - 1),
				  rGridSpacing = (back_probe_bed_position - front_probe_bed_position) / (auto_bed_leveling_rad_grid_points - 1);
			delta_grid_spacing[X_AXIS] = thGridSpacing;
			delta_grid_spacing[Y_AXIS] = rGridSpacing;
	#endif //POLAR_MESH
#else // !DELTA
        /**
      ...
      bool zig = true;
#else
      bool zig = (auto_bed_leveling_theta_grid_points & 1) ? true : false; //always end at [RIGHT_PROBE_BED_POSITION, BACK_PROBE_BED_POSITION]
...
pol2Cart(thCount*thGridSpacing,rCount*rGridSpacing,&xProbe,&yProbe);
#else
        yProbe = front_probe_bed_position + rGridSpacing * rCount;
        xProbe = left_probe_bed_position + thGridSpacing * thCount;
#endif// POLAR_MESH
            float distance_from_center = HYPOT(xProbe, yProbe);
            if (distance_from_center > DELTA_PROBEABLE_RADIUS) continue;

		  #else //!DELTA
        yProbe = front_probe_bed_position + yGridSpacing * yCount;
        xProbe = left_probe_bed_position + xGridSpacing * xCount;
          #endif //DELTA
#if ENABLED(POLAR_MESH)
          float measured_z;
  ...
  inline void gcode_G30() {
    ...
    setup_for_endstop_or_probe_move();
    uint8_t verbose_level = code_seen('V') ? code_value_int() : 1;
    ...
    SERIAL_PROTOCOLPGM("Bed X: ");
    SERIAL_PROTOCOL_F(current_position[X_AXIS] + X_PROBE_OFFSET_FROM_EXTRUDER,5);
    SERIAL_PROTOCOLPGM(" Y: ");
    SERIAL_PROTOCOL_F(current_position[Y_AXIS] + Y_PROBE_OFFSET_FROM_EXTRUDER,5);
    SERIAL_PROTOCOLPGM(" Z: ");
    SERIAL_PROTOCOL_F(measured_z,5);
    SERIAL_EOL;
...
static void report_current_position() {
  ...
  SERIAL_PROTOCOL_F(current_position[E_AXIS],5);
 ...
 * Output the current position to serial
 */
static void report_current_position2(float position[],long stepper_pos[]) {
  SERIAL_PROTOCOLPGM("x :");
  SERIAL_PROTOCOL_F(position[X_AXIS],5);
  SERIAL_PROTOCOLPGM(" y :");
  SERIAL_PROTOCOL_F(position[Y_AXIS],5);
  SERIAL_PROTOCOLPGM(" z :");
  SERIAL_PROTOCOL_F(position[Z_AXIS],5);
  SERIAL_PROTOCOLPGM(" e :");
  SERIAL_PROTOCOL_F(position[E_AXIS],5);
  long xpos = stepper_pos[X_AXIS],
       ypos = stepper_pos[Y_AXIS],
       zpos = stepper_pos[Z_AXIS];
...
#else //!POLAR_MESH
		int half = (AUTO_BED_LEVELING_GRID_POINTS - 1) / 2;
		x = delta_grid_spacing[X_AXIS]*(px-half);
		y = delta_grid_spacing[Y_AXIS]*(py-half);
#endif //POLAR_MESH
...
inline void gcode_M503() {
  ...
  inline void gcode_M851() {
    ...
    if (code_seen('D')) {
    	double_touch = code_value_bool();
    }
    if (code_seen('F')) {
    	probing_feedrate = code_value_linear_units();
    }
    if (code_seen('R')) {
    	probing_z_raise = code_value_axis_units(Z_AXIS);
    }
    SERIAL_EOL;
  }
...
void clamp_to_software_endstops(float target[3]) {
  ...
  void recalc_delta_settings(float radius, float diagonal_rod) {
	delta_tower1_x = arm_cos_f32(RADIANS( 210-120 + delta_tower_angle_trim[A_AXIS])) * (radius + delta_radius_trim_tower_1); // front left tower
	delta_tower1_y = arm_sin_f32(RADIANS( 210-120 + delta_tower_angle_trim[A_AXIS])) * (radius + delta_radius_trim_tower_1);
	delta_tower2_x = arm_cos_f32(RADIANS( 330-120 + delta_tower_angle_trim[B_AXIS])) * (radius + delta_radius_trim_tower_2); // front right tower
	delta_tower2_y = arm_sin_f32(RADIANS( 330-120 + delta_tower_angle_trim[B_AXIS])) * (radius + delta_radius_trim_tower_2);
	delta_tower3_x = arm_cos_f32(RADIANS( 90-120 + delta_tower_angle_trim[C_AXIS])) * (radius + delta_radius_trim_tower_3); // back middle tower
	delta_tower3_y = arm_sin_f32(RADIANS( 90-120 + delta_tower_angle_trim[C_AXIS])) * (radius + delta_radius_trim_tower_3);
...
#define maxdelta() (max3(delta[TOWER_1],delta[TOWER_2],delta[TOWER_3]))
  float delta_safe_distance_from_top() {
    float cartesian[3] = {
    ...
    float distance = maxdelta();
    cartesian[Y_AXIS] = LOGICAL_Y_POSITION(DELTA_PRINTABLE_RADIUS);
    inverse_kinematics(cartesian);
    return abs(distance - maxdelta());
  }
...
volatile int i = floor_x + half;
volatile int j = floor_y + half;
th=cartesian[X_AXIS];
r=cartesian[Y_AXIS];
#endif //POLAR_MESH
      ...
      volatile float t1 = i*delta_grid_spacing[0];
      volatile float t2 = (i+1)*delta_grid_spacing[0];
      volatile float r1 = j*delta_grid_spacing[1];
      volatile float r2 = (j+1)*delta_grid_spacing[1];
